...
class Household(ap.Agent):
    ...
    def buy_goods(self, C_star, market):
        C_max = min(self.M, C_star)
        suppliers = self.search_producers(market)
        for supplier in suppliers:
            C = min(C_max, supplier.pY * supplier.inv)
            market.buy_goods(C, self, supplier)
            C_max -= C
            if C_max <= 0:
                break

    def search_producers(self, market):
        found = market.search_suppliers(self.chiY)
        return found.sort("pY")
