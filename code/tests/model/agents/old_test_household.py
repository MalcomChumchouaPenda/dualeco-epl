...
def test_set_desired_consumption(household_as_consumer):
    ...
    assert household.C1_star == pytest.approx(expected_C1_star)
    assert household.C2_star == pytest.approx(expected_C2_star)
...
def test_sample_goods_suppliers(household_as_consumer):
    ...
    assert len(called_suppliers1) == household.chiY
    assert len(called_suppliers2) == household.chiY
...
def test_consume_at_desired_level(household_as_consumer):
    ...
    assert sum(purchased_amount1) == pytest.approx(household.C1_star)
    assert sum(purchased_amount2) == pytest.approx(household.C2_star)
